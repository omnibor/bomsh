...
def prune_bom_dir(bom_dir, blob_ids, destdir):
    '''
    Prune the artifact tree of an OmniBOR directory.
    :param bom_dir: OmniBOR dir generated by bomsh_create_bom.py script
    ...
    for blob in blob_ids:
        if blob in treedb:
            ...
            if "hash_tree" in value:
                # just need to delete the "hash_tree" to prune its subtree
                del value["hash_tree"]
    bomsh_objects_dir = get_or_create_dir(os.path.join(destdir, "objects"))
    ...
    save_json_db(os.path.join(bomsh_metadata_dir, "bomsh_omnibor_treedb"), treedb)
    save_json_db(os.path.join(bomsh_metadata_dir, "bomsh_omnibor_doc_mapping"), bom_db)
    # No need to change index_db, but we can truncate raw_logfile
    pkg_dbfile = os.path.join(bom_dir, "metadata", "bomsh", "bomsh_index_pkg_db")
    ...
    create_truncated_raw_logfile(raw_logfile, new_raw_logfile, blob_ids)
    verbose("===Done saving generated metadata docs to metadata dir: " + bomsh_metadata_dir, LEVEL_2)


def create_truncated_raw_logfile(raw_logfile, new_raw_logfile, blob_ids):
    '''
    Create new raw_logfile with some build fragments deleted for some blob IDs.
    ...
    with open(raw_logfile, 'r') as f:
        ...
        for line in f:
            if line.startswith("outfile: "):
                ...
                if found_record:
                    found_record = False
                    of.write("==== Deleted one build fragment for outfile checksum: " + checksum)
                    continue
            if not found_record:
                of.write(line)
    of.close()
...
def prune_bom_dirs(bom_dirs, blob_ids, destdir):
    """
    Prune OmniBOR subtrees for a list of blobs, that is, these blobs will become leaf nodes.
