...
def analyze_files(rpm_file, unpack_dir):
    ...
    for idx, f in enumerate(file_data):
        ...
        file_path = os.path.join(unpack_dir, file_name)
        file_sha1 = calculate_file_checksum(file_path, hash_algorithm=ChecksumAlgorithm.SHA1)
        file_rec = File(
            name=file_name,
            ...
            file_types=[get_spdx_file_type(file_path)],
            checksums=[
...
def build_all_spdx(env_data, tree_db):
    ...
    for rpm_path in rpms:
        gitoid = get_pkg_gitoid(str(rpm_path))
        (doc_uuid, pkg_bom_id, spdx_doc, validation_messages) = build_sbom(str(rpm_path), env_data)
